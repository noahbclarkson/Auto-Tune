...
if (!c.includes(old)) {
    ...
    const old2 = `/// This test runs both fixes against the control (tier3=30, min_int=0.0) on 2 seeds\r\n/// to confirm whether the fix resolves the 60-day instability.\r\nfn run_sixty_day_fix_test() {`;
    if (c.includes(old2)) {
        c = c.replace(old2, '');
        fs.writeFileSync('src/main.rs', c);
        console.log('SUCCESS (CRLF variant)');
        process.exit(0);
    }
...
const new_content = c.replace(old, new_fn);
fs.writeFileSync('src/main.rs', new_content);
console.log('SUCCESS: 90-day function inserted');

...
export function RegisterServerModal() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isNaN(parsedCount) || parsedCount < 0) {
      setError('Player count must be a non-negative number.');
      return;
    }

    setLoading(true);
    const result = await registerServer(name.trim(), parsedCount);
    setLoading(false);
            ...
            {success ? (
              ...
              <div className="space-y-4">
                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
                  ...
                  <p className="text-xs text-gray-400">ID: <span className="font-mono text-gray-300">{success.id}</span></p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label htmlFor="player-count" className="block text-sm font-medium text-gray-300 mb-1.5">
                    Player Count
                  </label>
                  <input
                    id="player-count"

...
- `public-site/` (15 routes): Setup Wizard implemented, all landing sections done, OG image generated
...
- ⚠️ SPEC-SERVER-SETUP-WIZARD — Fully implemented in public-site (8 components, wizard-data, route)

**rewrite-2 has unbuilt changes:** The Admin Advice feature (`EconomyAdvisor.java`) and `/at admin advice` command were added but the public-site landing page hasn't incorporated them yet (no update to landing page sections).
...
public-site shows GitHub stars but no real server count, trade volume, or active economy statistics. The API server's aggregated stats are never shown publicly. A live ticker ("X servers | Y trades today | Z items priced") would be powerful social proof.
...
Consider: GitHub Sponsors, optional "Auto-Tune Pro" features (extended analytics, Discord alerts), or a voluntary "support the network" donation link on public-site.
...
**4. "Economy Health" embeddable badge (public-site) — 2h**
Static HTML badge generated by `/health-badge`. Shows a server label and optional dashboard link without depending on an unshipped public widget runtime.
...
**6. Live stats ticker on landing page (public-site) — 2h**
...
Spanish, Portuguese, Chinese language switcher for both web/ and public-site/. All strings externalized. Auto-Tune has international reach — Spanish-speaking Minecraft servers would benefit greatly.
...
2. Set `NEXT_PUBLIC_API_URL` in public-site/.env
3. Build + push public-site
4. Build server key issuance portal (simple web form + email)

Until #1 is done, public-site's true-prices, exchange-rates, and servers pages show empty/mock data.

...
| `/auction sell <price>` | Post a sell limit order at your price |
| `/auction buy <order-id>` | Fill a seller's order at their price |
...
Hold the item you want to sell, then run `/auction sell <price>`. Your items go into escrow. A sell order stays active for 72 hours (default) or until it is fully filled. The order will not affect your inventory until a buyer crosses your price.

```
/auction sell 315
# Posts your held item as a sell order at $315 per unit
```

### Filling a Buy Order

Browse with `/auction browse` to see active sell orders. To buy from a seller, run `/auction buy <order-id>`. The order ID is shown in the browse output. Payment is instant — the seller receives coins, you receive the items.

```
/auction buy abc123-def456-...
# Fills the seller's order at their price
```
...
When an order expires (72 hours by default), sell orders' items remain in escrow. Run `/auction reclaim` to get them back.
...
Sell orders that expire leave items in escrow. These are not returned automatically — players must run `/auction reclaim`. If you see many expired orders, consider reminding players about reclaim.

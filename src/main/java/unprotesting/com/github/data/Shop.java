...
public class Shop implements Serializable {
  ...
  protected void loadConfiguration(ConfigurationSection config, String sectionName) {
    ...
    Format.getLog().finest("Locked: " + this.locked);
    customSpd = config.getDouble("sell-price-difference", -1);
    Format.getLog().finest("Custom SPD: " + this.customSpd);
    volatility = config.getDouble("volatility", Config.get().getVolatility());
    Format.getLog().finest("Volatility: " + this.volatility);
    section = sectionName;
    Format.getLog().finest("Section: " + this.section);
    maxBuys = config.getInt("max-buy", -1);
    Format.getLog().finest("Max Buys: " + this.maxBuys);
    maxSells = config.getInt("max-sell", -1);
    Format.getLog().finest("Max Sells: " + this.maxSells);
    updateRate = config.getInt("update-rate", 1);
    Format.getLog().finest("Update Rate: " + this.updateRate);
    ...
    if (section == null) {
      Format.getLog().warning("Shop " + config.getName() + " was loaded with no section!");
    }
  ...
  public void addBuys(UUID player, int buyCount) {
    if (recentBuys.containsKey(player)) {
      ...
      Format.getLog().finest("Recent buys: " + recentBuys.get(player));
    } else {
      recentBuys.put(player, buyCount);
      Format.getLog().finest("New recent buys: " + recentBuys.get(player));
    }
  ...
  public void addSells(UUID player, int sellCount) {
    if (recentSells.containsKey(player)) {
      ...
      Format.getLog().finest("Recent sells: " + recentSells.get(player));
    } else {
      recentSells.put(player, sellCount);
      Format.getLog().finest("New recent sells: " + recentSells.get(player));
    }
    this.sells[size - 1] = sellCount + sells[size - 1];
    Format.getLog().finer("Increased sells by " + sellCount + " to " + sells[size - 1]);
    Format.getLog().finest("Updated at time period " + (size - 1));
  }
  ...
  public void addAutosell(UUID uuid, int count) {
    if (autosell.containsKey(uuid)) {
      ...
      Format.getLog().finest("Autosell: " + autosell.get(uuid) + " for " + uuid);
    } else {
      autosell.put(uuid, count);
      Format.getLog().finest("New autosell: " + autosell.get(uuid) + " for " + uuid);
    }
  }

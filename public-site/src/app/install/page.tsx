...
export default function InstallPage() {
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <section className="mb-12">
        <h2 className="text-xl font-bold text-white mb-2">Add a static economy badge to your forum</h2>
        <p className="text-gray-400 text-sm mb-6 leading-relaxed">
          The public site is exported as static files, so it does not host arbitrary per-server widget URLs.
          Use the badge generator instead: it creates a self-contained HTML badge that links to your bundled Auto-Tune dashboard.
        </p>

        <div className="grid md:grid-cols-2 gap-6 items-start">
          <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
            <p className="text-xs text-gray-500 uppercase tracking-wide mb-4">Badge preview</p>
            <div className="flex justify-center">
              <div className="rounded-lg border border-emerald-700/60 bg-gray-950 px-4 py-2 font-mono text-xs">
                <span className="text-gray-300 font-semibold">Cobblestone SMP</span>
                <span className="mx-2 text-gray-600">-</span>
                <span className="text-emerald-400">Economy Healthy</span>
              </div>
            </div>
          </div>

          <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
            <h3 className="text-sm font-semibold text-white mb-3">How to use it</h3>
            <div className="space-y-3 text-sm">
              {[
                ['1', 'Enable the plugin dashboard', 'In config.yml, enable the web server and set the port you want players or admins to open.'],
                ['2', 'Generate the badge', 'Enter your server name and dashboard URL on the badge page. Choose compact, standard, or detailed output.'],
                ['3', 'Paste the HTML', 'The output is a small static snippet, so it works on forums, static sites, and README pages.'],
              ].map(([step, title, body]) => (
                <div key={step} className="flex items-start gap-3">
                  <span className="w-5 h-5 rounded-full bg-emerald-900 border border-emerald-700 flex items-center justify-center text-[10px] font-mono text-emerald-400 shrink-0 mt-0.5">{step}</span>
                  <div>
                    <p className="text-gray-300 font-medium text-xs">{title}</p>
                    <p className="text-gray-500 text-xs mt-0.5">{body}</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-4 rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-xs text-amber-400/80">
              <strong>Static by design.</strong> A live widget can be added later when the public site has a server runtime.
            </div>
            <Link
              href="/health-badge"
              className="mt-4 inline-flex items-center gap-2 text-xs text-emerald-400 hover:text-emerald-300 transition-colors"
            >
